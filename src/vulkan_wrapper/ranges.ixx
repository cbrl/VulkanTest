...
public:
    ...
    private:
        ...
        template <typename Cont, r::range Rng> requires container_convertible<Cont, Rng>
        ...
        constexpr static auto impl(Rng&& rng, Args&&...args) {
            return to<Cont, Args...>(
                rng | r::views::transform([](auto&& elem) {
    ...
    public:
        template <typename Rng> requires r::input_range<Rng> && recursive_container_convertible<container_t<C, Rng, Args...>, Rng&&>
...
export {
    template <template <typename...> class ContT, typename... Args, detail::to_container = {}> requires(!std::ranges::range<Args> && ...)
    constexpr auto to(Args&&...args) -> detail::to_container_fn<detail::wrap<ContT>, Args...> {
        detail::to_container_fn<detail::wrap<ContT>, Args...> fn;
        fn.args = std::forward_as_tuple(std::forward<Args>(args)...);
        return fn;
    }

    template <template <typename...> class ContT, std::ranges::input_range Rng, typename... Args> requires std::ranges::range<Rng>
    ...
    template <typename Cont, typename... Args, detail::to_container = {}> requires(!std::ranges::range<Args> && ...)
    ...
    template <typename Cont, std::ranges::input_range Rng, typename... Args> requires detail::recursive_container_convertible<Cont, Rng>
    constexpr auto to(Rng&& rng, Args&&...args) -> Cont {
